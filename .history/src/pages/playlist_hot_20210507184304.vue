...
<script lang="ts">
import { PLAYLIST_CATLIST, PLAYLIST_HOT, PLAYLIST_INFO,HIGHT_QUALITY_PL } from "@/api/index";
...
export default {
  setup() {
    ...
    onBeforeMount(async () => {
      ...
      const allList = await HIGHT_QUALITY_PL(100).then(res => { console.log(res) })
      state.hotPlayList = (hotPlayList as any).data.playlists;
      state.playListInfo = (playListCats as any).data;
      state.allList = (allList as any).playlists;
